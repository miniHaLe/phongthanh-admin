...
import { administrativeDisplayName } from './customer-form-values'
import {
  findUniqueExactCommune,
  normalizeAdministrativeName,
} from './customer-commune-search'

const MAX_RESULTS = 80

interface CustomerCommuneComboboxProps {
  id: string
...
export function CustomerCommuneCombobox({
  id,
  communes,
  provinceCode,
  value,
  ...
  useEffect(() => {
    if (!open) setQuery(administrativeDisplayName(selected))
  }, [open, selected])
  ...
  function pick(commune: VietnamCommune) {
    onSelect(commune)
    setQuery(administrativeDisplayName(commune))
    setOpen(false)
  }
      ...
      <div className="relative">
        <Input
          ...
          onKeyDown={(event) => {
            ...
            if (event.key === 'Enter') {
              event.preventDefault()
              const exact = findUniqueExactCommune(query, scopedCommunes)
              const option =
                exact ?? (activeIndex >= 0 ? options[activeIndex] : undefined)
