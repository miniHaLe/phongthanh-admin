...
- all six real resources enabled;
...
Migration `0002_backfill-khach-hang-address-codes.sql` follows `0001`. The old
`0001_cool_sunspot.down.sql` path is unsafe after `0002` because it does not
reconcile the later migration ledger/data state. Do not use the `0001` down
script as a release rollback. Use application-only rollback or restore a full,
