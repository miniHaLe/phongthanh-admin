...
with a NestJS + Postgres + Drizzle API under `api/`. JWT auth and six resources
use the real API: `khach-hang`, `nha-san-xuat`, `san-pham`, `model`, `ngan-hang`,
and read-only `dia-ly`. Customer quick-create, including dealer types, persists
through the same real customer contract. Repair tickets and most operational
domains remain deterministic client mock data.
...
- `.github/workflows/deploy-pages.yml` uses Node 24, `npm ci`, hash routing, the
  GitHub Pages base path, and all six real resources.
...
- 187 Vitest files / 633 frontend tests.
- 8 Jest API suites / 64 tests.
