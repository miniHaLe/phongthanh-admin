...
tại yêu cầu sáu resource thật trong production:
...
Quick-create Đại lý và Bán hàng giữ nguyên implementation cũ, ngoài phạm vi
release này.
...
| `src/api/*` + `config/crud-configs/*`                    | Khách hàng, Nhà sản xuất, Sản phẩm, Model, Ngân hàng, Địa lý khi bật real resource | —                                 |
| `src/mock/masterdata/*` + `config/crud-configs/*`        | Các danh mục còn lại và fallback dev/test                                          | —                                 |
